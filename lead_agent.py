...
def scrape_bing(query, lead_type="B2B"):
    ...
    try:
        ...
        res  = requests.get(url, headers=headers, proxies=get_next_proxy(), timeout=15)
        ...
        for site_url in urls:
            ...
            for suffix in ["", "/contact", "/contact-us", "/about", "/about-us", "/team"]:
                try:
                    r = requests.get(site_url.rstrip("/") + suffix, headers=headers, timeout=8, proxies=get_next_proxy())
    ...
    try:
        ...
        for biz_url in links:
            ...
            try:
                ...
                website_el = soup2.select_one("a[href*='biz_redir']")
                if website_el:
                    site_url = website_el.get("href", "")
...
async def get_emails_from_website(page, url):
    for suffix in ["", "/contact", "/contact-us", "/about", "/about-us", "/team"]:
        try:
            await page.goto(url.rstrip("/") + suffix, wait_until="domcontentloaded", timeout=10000)
...
async def run_agent():
    ...
    async with async_playwright() as p:
        ...
        while (time.time() - agent_start) < (RUN_HOURS * 3600):
            ...
            print(f"\n{'='*60}")
            print(f"[CYCLE {cycle}] Elapsed: {elapsed} mins | Total: {total_saved}")
            print(f"{'='*60}")
    ...
    print("\n" + "=" * 60)
    print(f"[DONE] Total leads: {total_saved}")
